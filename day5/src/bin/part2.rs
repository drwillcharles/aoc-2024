...
fn check_order(after_before: &MultiMap<i32,i32>, updates_after: &[i32], val: &i32) -> bool {
        ...
        match check {
            Some(number) => {
                ...
                for before in number {
                    if updates_after.contains(before) {
                        println!("Invalid {:?} in {:?}", number, updates_after);
                        return false;
                    }
                }
            }
...
fn check_updates(after_before: &MultiMap<i32,i32>, updates: &Vec<Vec<i32>>) -> i32 {
    ...
    for update in updates {
        ...
        for (i, val) in update.iter().enumerate() {
            ...
            valid = check_order(&after_before, &update[i..], &val);
